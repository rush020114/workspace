...
CREATE TABLE USERS(
	...
	, USER_ROLE VARCHAR(10) NOT NULL DEFAULT 'USER' #사용자 유형 : USER(개인), ADMIN(관리자)
...
CREATE TABLE APPLICATION(
	...
	, FARM_NAME VARCHAR(20) NOT NULL #농장 이름
	...
	, APPL_CONTENT VARCHAR(100) #신청 내용
	, REG_DATE DATETIME DEFAULT SYSDATE() # 서비스가입일자
	, APPR_DATE DATETIME #서비스등록일자
...
CREATE TABLE question_img(
	...
	, QST_ID INT REFERENCES question(QST_ID) ON DELETE CASCADE # 이미지 등록자
);
...
CREATE TABLE ANSWER(
	...
	, USER_ID VARCHAR(10) REFERENCES USERS (USER_ID) # 작성자(관리자)
