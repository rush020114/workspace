...
FROM test_book B
WHERE (SELECT BORROWER_NAME
		FROM RENTAL
		WHERE BOOK_ID = B.BOOK_ID) IS NOT NULL;
...
SELECT TITLE
	...
	, (SELECT COUNT(RENTAL_ID) 
		FROM rental
		WHERE BOOK_ID = B.BOOK_ID) 대출횟수
	, IF((SELECT COUNT(RENTAL_ID) 
			FROM rental 
			WHERE BOOK_ID = B.BOOK_ID) > 0, '대출기록있음', '대출기록없음')
FROM test_book B;
...
SELECT TITLE
	...
	, (SELECT COUNT(RENTAL_ID) 
		...
		WHERE BOOK_ID = B.BOOK_ID) 대출횟수
	, CASE
		WHEN (SELECT COUNT(RENTAL_ID)
				FROM RENTAL
				WHERE BOOK_ID = B.BOOK_ID) > 0 THEN '대출기록있음'
		ELSE '대출기록없음' END 대출여부
FROM test_book B;
