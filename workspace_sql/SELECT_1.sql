-- 이것은 주석입니다.
#이것도 주석입니다.
# 실행(F9) : 현재 열려있는 쿼리탭의 모든 실행문을 실행
# 선택실행(Ctrl + F9) : 드래그한 영역만 실행
# 현재쿼리실행 (Shift + Ctrl + F9) : 현재 커서가 올라가 있는 쿼리를 실행
...
# 2. EMP테이블에 저장된 사원과 사원명을 조회
SELECT EMPNO, ENAME FROM emp;

# 3. EMP테이블에 저장된 사원들의 모든 컬럼 정보 조회
SELECT * FROM emp;
...
# EMP 테이블에서 급여가 300이상이고 사번은 1003번인 사원들의 
# 모든 컬럼 조회
# 그리고(AND), 이거나(OR)
SELECT *
FROM emp
WHERE SAL >= 300 AND EMPNO >= 1003;

# EMP 테이블에서 직급이 사원이 아니고, 급여는 300이상이면서
# 커미션은 NULL인 직원의 모든 정보 조회
# NULL 여부는 =, != 등과 같은 등호기호로 판단하지 않는다.
# NULL 체크는 IS, IS NOT 키워드를 사용한다.
SELECT *
FROM emp
WHERE JOB != '사원' AND SAL >= 300 AND COMM IS NOT NULL;
